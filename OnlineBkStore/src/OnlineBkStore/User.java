...
public class User
{
    ...
    public void login()
    {
     ...
     {
      con = getConnection();
      ps = con.prepareStatement("select userid,email,phone,address from user where uname = ? and pwd= ?");
 ...
 public String updateProfile(String newpwd) 
 {
   ...
   {
     con = getConnection();
     String cmd = "update user set email=?, phone  = ? , address =? ";
 ...
 {
    PreparedStatement ps = null;
    ps = con.prepareStatement("select IFNULL( max(userid),0) + 1 from user");
    ResultSet rs = ps.executeQuery();
    rs.next();
